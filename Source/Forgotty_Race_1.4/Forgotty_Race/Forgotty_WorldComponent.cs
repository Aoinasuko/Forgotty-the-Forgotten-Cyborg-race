...
{
	public class Forgotty_WorldComponent : WorldComponent
	{

		public Forgotty_WorldComponent(World world)
			: base(world)
		{
		}
		...
		public override void WorldComponentTick()
		{
			...
			{
				// 現在のバージョン
				if (Forgotty_Config.Updatever < Forgotty_Config.ver)
				{
					Forgotty_Config.Updatever = Forgotty_Config.ver;
					Dialog_Update dialog = new Dialog_Update();
					Find.WindowStack.Add(dialog);
					LoadedModManager.GetMod<Forgotty_Settings>().WriteSettings();
				}
			}
		}
	...
	public class Dialog_Update : Window
	{
		...
		{
			Texture2D texture_update = ContentFinder<Texture2D>.Get("Forgotty/UI/Update");
			Texture2D texture_nasuko = ContentFinder<Texture2D>.Get("Forgotty/UI/Fecard");
			...
			listingStandard.Label("Forgotty.Update.Title".Translate());
			...
			listingStandard.Label("Forgotty.Update.label".Translate());
			listingStandard.End();
